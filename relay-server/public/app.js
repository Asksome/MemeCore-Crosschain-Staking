...
// Network switch helper (used by index.html onclick="switchNetwork()")
async function switchNetwork() {
    try {
        const eth = getTrueMetaMaskProvider();
        if (!eth) {
            ui.showAlert("Wallet Error", "MetaMask is not available (or Phantom is blocking it).", true);
            return;
        }

        const chainIdNum = Number(cfg.MEMECORE_CHAIN_ID || 43521);
        ...
        await eth.request({
            ...
            params: [{
                chainId: chainIdHex,
                chainName: "MemeCore Formicarium Testnet",
                rpcUrls: [cfg.MEMECORE_RPC || "https://rpc.formicarium.memecore.net"],
                nativeCurrency: { name: "MemeCore", symbol: "M", decimals: 18 },
                blockExplorerUrls: ["https://formicarium.memecorescan.io/"],
            }],
        });

        ui.showAlert("Network", "MemeCore Testnet network request sent to MetaMask.");
    } catch (e) {
        console.error(e);
        ui.showAlert("Network Error", e.message || String(e), true);
    }
}
